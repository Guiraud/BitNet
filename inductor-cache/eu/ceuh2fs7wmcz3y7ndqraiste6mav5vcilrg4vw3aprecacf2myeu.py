...
cpp_fused_abs_clamp_div_max_mul_reciprocal_round_0 = async_compile.cpp_pybinding(['const float*', 'float*', 'float*'], '''
            ...
            {
                {
                    ...
                    for(int64_t x1=static_cast<int64_t>(0LL); x1<static_cast<int64_t>(2560LL); x1+=static_cast<int64_t>(4LL))
                    {
                        {
                            if(C10_LIKELY(x1 >= static_cast<int64_t>(0) && x1 < static_cast<int64_t>(2560LL)))
                            {
                                auto tmp0 = at::vec::Vectorized<float>::loadu(in_ptr0 + static_cast<int64_t>(x1 + 2560LL*x0), static_cast<int64_t>(4));
                                auto tmp1 = tmp0.abs();
                ...
                {
                    {
                        if(C10_LIKELY(x1 >= static_cast<int64_t>(0) && x1 < static_cast<int64_t>(2560LL)))
                        {
                            ...
                            tmp16.store(out_ptr1 + static_cast<int64_t>(x1 + 2560LL*x0));
                        }
                    }
                }
            }
...
def call(args):
    arg0_1, = args
    args.clear()
    assert_size_stride(arg0_1, (1, 34, 2560), (87040, 2560, 1))
    buf0 = empty_strided_cpu((1, 34, 1), (34, 1, 34), torch.float32)
    buf2 = empty_strided_cpu((1, 34, 2560), (87040, 2560, 1), torch.float32)
    cpp_fused_abs_clamp_div_max_mul_reciprocal_round_0(arg0_1, buf0, buf2)
    del arg0_1
    return (buf2, )
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    arg0_1 = rand_strided((1, 34, 2560), (87040, 2560, 1), device='cpu', dtype=torch.float32)
